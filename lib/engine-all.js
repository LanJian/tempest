  ...
  window.Tween = (function() {
    ...
    Tween.prototype.init = function() {
      ...
      for (v = _i = 0, _len = _ref.length; _i < _len; v = ++_i) {
        k = _ref[v];
        _results.push(this.initialProps[k] = this.obj[k]);
      }
      return _results;
    };
    ...
    Tween.prototype.updateProps = function(obj, props, initProps) {
      var initialValue, k, ratio, v, _i, _len, _results;
      console.log('update Tween');
      if (this.finished) {
        return;
      }
      _results = [];
      for (v = _i = 0, _len = props.length; _i < _len; v = ++_i) {
        k = props[v];
        ratio = this.curTime * 1.0 / this.duration;
        if ($.isNumeric(v)) {
          initialValue = initProps[k];
          _results.push(obj[k] = ratio * (v - initialValue));
        } else {
